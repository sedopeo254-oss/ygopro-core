...
bool field::process(Processors::SelectChain& arg) {
	...
	auto write_chain = [&](auto* out_message, size_t chain_index) {
		...
		effect* peffect = ch.triggering_effect;
		card* pcard = peffect->get_handler();
		out_message->template write<uint32_t>(pcard->data.code);
		out_message->write(pcard->get_info_location());
		out_message->template write<uint64_t>(peffect->description);
		out_message->template write<uint8_t>(peffect->get_client_mode());
	};
	if(arg.step == 0) {
		returns.set<int32_t>(0, -1);
		if((playerid == 1) && is_flag(DUEL_SIMPLE_AI)) {
			...
			if(split_team_prompt) {
				for(uint8_t logical_player = 0; logical_player < 3; ++logical_player) {
					...
					if(found != core.select_chains.end())
						arg.logical_players.push_back(logical_player);
				}
			}
		}
		...
		if(split_team_prompt && !arg.logical_players.empty()) {
			...
			selecting_player = static_cast<uint8_t>(logical_player + 2);
		}
		...
		if(split_team_prompt && !arg.logical_players.empty()) {
			...
			message->write<uint32_t>(core.select_chains.size());
			for(size_t chain_index = 0; chain_index < core.select_chains.size(); ++chain_index)
				write_chain(message, chain_index);
		}
		return FALSE;
	} else {
		if(split_team_prompt && !arg.logical_players.empty()) {
			...
			if(!forced && selected == -1) {
				++arg.logical_index;
				if(arg.logical_index < arg.logical_players.size()) {
					arg.step = Processors::restart;
					return FALSE;
				}
				return TRUE;
			}
			if(selected < 0 || selected >= static_cast<int32_t>(arg.chain_indices.size())) {
				pduel->new_message(MSG_RETRY);
				return FALSE;
			}
			returns.set<int32_t>(0, static_cast<int32_t>(arg.chain_indices[selected]));
			return TRUE;
		}
